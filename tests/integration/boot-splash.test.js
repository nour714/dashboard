...
describe('Boot Splash Screen Verification', () => {
  ...
  const baseCssPath = path.join(rootDir, 'frontend', 'styles', 'base.css');
  const appJsPath = path.join(rootDir, 'frontend', 'js', 'app.js');
  const enLocalePath = path.join(rootDir, 'frontend', 'js', 'i18n', 'locales', 'en.js');
  const arLocalePath = path.join(rootDir, 'frontend', 'js', 'i18n', 'locales', 'ar.js');
  ...
  it('1. index.html contains static boot splash markup in #app for zero-blank initial paint', () => {
    ...
    assert.ok(indexHtml.includes('id="boot-step-user"'), 'index.html must have #boot-step-user');
    ...
    console.log('  ✓ Static luxury boot splash markup present in #app with 4-step card');
  });
  ...
  it('2. styles/base.css defines comprehensive styles for boot splash', () => {
    ...
    console.log('  ✓ styles/base.css defines all luxury boot splash components, active/done states, and progress bar');
  });

  it('3. js/app.js drives real honest 4-step progress and removes splash on shell render', () => {
    ...
    assert.ok(appJs.includes('boot-step-user'), 'app.js must update boot-step-user');
    ...
    console.log('  ✓ js/app.js drives honest 4-step async bootstrap progress');
  });
  ...
  it('4. en.js and ar.js contain complete bootSplash translation dictionary', () => {
    ...
    assert.ok(enContent.includes('loadingUserData:'), 'en.js must have loadingUserData');
    ...
    assert.ok(arContent.includes('loadingUserData:'), 'ar.js must have loadingUserData');
