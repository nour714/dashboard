...
async function runExtractionTests() {
  ...
  globalThis.fetch = async (url, options) => {
    ...
    assert(body.generationConfig?.responseSchema?.properties?.returnFlightNumber?.description?.includes('FIRST return segment ONLY'), 'returnFlightNumber schema description specifies first return segment only');
  ...
  assert(normalizedAirports.flightNumber === 'ET 0453', 'Multi-segment flight numbers normalized to first segment only (ET 0453)');
  ...
  assert(toSingleFlightNumber('MS 986') === 'MS 986', 'toSingleFlightNumber leaves single flight number intact');
  ...
  const ticketCreateSrc = fs.readFileSync(path.resolve('frontend/js/pages/ticket-create.js'), 'utf8');
  ...
  const codeFilesToCheck = [
    ...
    'backend/src/services/ticket-extraction.service.js',
    'backend/src/routes/ticket.routes.js',
    'frontend/js/pages/ticket-create.js',
    'frontend/js/services/ticket-service.js',
    '.env.example'
  ];
