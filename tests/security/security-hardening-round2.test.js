...
import { NotFoundError, ForbiddenError, UnauthorizedError } from '../../backend/src/domain/errors.js';
...
async function runSecurityHardeningRound2Tests() {
  ...
  try {
    ...
    const settingsJsContent = fs.readFileSync(path.resolve('frontend/js/pages/settings.js'), 'utf8');
    assert(
