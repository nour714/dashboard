...
import { TicketService } from '../../backend/src/services/ticket.service.js';
import { CustomerService } from '../../backend/src/services/customer.service.js';
...
import { UnauthorizedError, BusinessRuleError } from '../../backend/src/domain/errors.js';
import fs from 'fs';
...
async function runSecurityFixesTests() {
  ...
  const envJsContent = fs.readFileSync('backend/src/config/env.js', 'utf8');
  assert(envJsContent.includes('[JWT_SECRET]') || envJsContent.includes('JWT_SECRET'), 'backend/src/config/env.js enforces check on production with insecure default secret');
  assert(envJsContent.includes('[BOOTSTRAP_ADMIN_PASSWORD / DEFAULT_ADMIN_PASSWORD]') || envJsContent.includes('DEFAULT_ADMIN_PASSWORD'), 'backend/src/config/env.js enforces check on production with default password');
  ...
  const securityJsContent = fs.readFileSync('backend/src/middleware/security.js', 'utf8');
  assert(!securityJsContent.includes('isVercelOrigin'), 'backend/src/middleware/security.js does not contain wildcard isVercelOrigin');
  assert(securityJsContent.includes("env.NODE_ENV !== 'production'"), 'backend/src/middleware/security.js restricts localhost to non-production');

  console.log('\n--- 4. Health Endpoint Leakage Prevention ---');
  const routesIndexContent = fs.readFileSync('backend/src/routes/index.js', 'utf8');
  assert(!routesIndexContent.includes('detectedEnvVars'), 'backend/src/routes/index.js does NOT leak detectedEnvVars');
  assert(routesIndexContent.includes("process.env.NODE_ENV === 'production'"), 'backend/src/routes/index.js checks NODE_ENV for hiding diagnostics in production');

  console.log('\n--- 5. httpOnly Cookies & In-Memory Tokens ---');
  const authControllerContent = fs.readFileSync('backend/src/controllers/auth.controller.js', 'utf8');
  ...
  const apiClientContent = fs.readFileSync('frontend/js/services/api-client.js', 'utf8');
  ...
  const schemaPrismaContent = fs.readFileSync('database/prisma/schema.prisma', 'utf8');
