...
import { setSupabaseClient } from '../../backend/src/config/storage.js';
...
import {
  ...
  ValidationError,
  BusinessRuleError
} from '../../backend/src/domain/errors.js';
...
async function runSecurityHardeningRound3Tests() {
  ...
  console.log('\n--- 6. Passport Document Security ---');
  {
    ...
    try {
      ...
      threwOversized = err instanceof ValidationError && err.message.includes('15MB');
    }
    assert(threwOversized, 'uploadPassportDocument rejects file exceeding 15MB limit');
